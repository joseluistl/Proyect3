...
class SentClass:
    ...
    def clean_text(self, text_col):
        ...
        [10 points]
        '''
        self.data = (self.data.assign(words= self.data[text_col].map(lambda x: utils.proc_text(x, self.sw))))
    ...
    def gen_voc(self, min_count=500):
        ...
        index. The vocabulary must contain words that have at least *min_count* entries within the corpus.
    ...
    def build_data_features(self):
        ...
        [10 points]
        '''
        self.data = (self.data.assign(bigrams=self.data["words"].map(lambda x: utils.get_bigrams(x))))
        self.data = (self.data.assign(neg_words=self.data["words"].map(lambda x: utils.prefix_neg(x))))
        self.data = (self.data.assign(neg_bigrams=self.data["neg_words"].map(lambda x: utils.get_bigrams(x))))
    ...
    def get_feature_voc(self):
        ...
        for i, feature in enumerate(feature_cols):
            ...
            # YOUR CODE GOES HERE
            pos_voc, neg_voc = self.gen_voc_features(feature)
            cad = "f"+str(i)
